...
func UcurlToolApi(args []string) {
	if len(args) != 4 {
		log.Fatal("invalid input args, Example: ./ckyn ucurl get /var/run/docker.sock http://127.0.0.1/info \"\"")
	}
...
func DcurlToolApi(args []string) {
	if len(args) != 3 {
		log.Fatal("invalid input args, Example: ./ckyn dcurl get http://127.0.0.1:2375/info \"\"")
	}
