...
import (
	...
	"github.com/Yanxinwu946/Ckyn/pkg/errors"
)
...
func CheckUnpriUserNS() error {
	...
	if err != nil {
		return &errors.RuntimeError{Err: err, CustomMsg: "check prerequisites error."}
	}

	if strings.TrimSuffix(string(data), "\n") != "1" {
		return &errors.RuntimeError{Err: nil, CustomMsg: "host os does NOT enable unprivileged user namespace."}
	}
